...
Код главы: [bridges.go](../../lessons/reference/bridges/bridges.go) · [example_test.go](../../lessons/reference/bridges/example_test.go).


> Цель главы: Распознавать библиотечные преобразования и адаптеры.

Некоторые библиотечные операции внешне похожи на конверсию, но выполняют дополнительную работу. Сравним поиск ошибки в цепочке и превращение функции в обработчик HTTP.

## `errors.As`: извлечение ошибки из дерева
