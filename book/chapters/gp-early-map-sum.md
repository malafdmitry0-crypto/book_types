...
Этот код берётся из [исполняемого примера](../../algorithms/example_test.go). `texts` создан до Map, остаётся `[]string` после Map и не требует assertion. Здесь `texts` создан через make и остаётся не-nil пустым срезом даже при nil-источнике. Сохранять ли nil-результат для nil-источника, решает вызывающий при создании Target, а не общий цикл.
...
У BoxInt корректный ноль — нулевая структура. У Fraction нулевая структура невалидна: знаменатель 0 не проходит Validate, а нулём служит 0/1. У Money нулю нужна валюта. Поэтому общий Sum не может просто создать «нулевое значение какого-то типа» и считать вопрос решённым.
