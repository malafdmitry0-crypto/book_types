...
## Как reflect связан с интерфейсом
...
На схеме отдельный минимальный пример: User с единственным полем Age. TypeOf исследует конкретный тип внутри any, ValueOf даёт доступ к переданному значению. В алгоритмах ниже таким значением будет весь срез или функция. [Подробный разбор с картинками: указатели, Elem, Set и сам интерфейс](08-reflection.md).

## Меняем границу API алгоритма
